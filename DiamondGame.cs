...
/// <summary>
/// DIAMOND GAME — Visual Upgrade v3.0 "NETFLIX TIER"
///
/// NEW IN V3 (ART DIRECTOR PASS):
///
///  DEPTH & MATERIALITY:
///    - Wet floor reflection layer with animated ripples (city silhouette mirrored)
///    - Three distinct Z-planes with CanvasGroup alpha separation (far 0.55 / mid 0.80 / near 1.0)
///    - Per-panel specular top-left edge highlight simulating near-light catch
///    - Digit box glass-metal material: top bevel + inset shadow + diagonal specular sweep
///    - Panel inner glow responds dynamically to game state (normal→danger→critical)
///    - Button surface wear tracking: 3+ presses darkens, 5+ adds bright edge highlight
///
///  ENVIRONMENTAL STORYTELLING:
///    - Ghost attempt layer: faint (2-4% alpha) digit outlines from "previous players"
///    - Rain layer: 80 animated vertical streaks for authentic Tokyo night atmosphere
///    - Spectator presence: live spectator count updating + observer silhouette corners
///    - Digit button wear accumulation across the session
///    - Handwritten chalk-mark annotations near attempt history
///    - Panel corners accumulate visual "damage" at high tension levels
///
///  PANEL BREATHING SYSTEM:
///    - All panels do 4-6s sine color sweep (Primary→Secondary→Primary) when idle
///    - Corner brackets pulse at staggered 1.2s intervals even during idle
///    - Timer corona throbs at low-health 6% alpha even at full health
///    - Status bar has animated sweep underline on new messages
///
///  TIMER PHYSICAL TICKING:
///    - Discrete 1-second mechanical tick (snap, not glide) with DOScale punch
///    - Double-tick at ≤10s, triple-tick at ≤5s
///    - Tick sound synced exactly to the snap
///
///  WORLD STATE REACTION:
///    - Win: entire background shifts hue toward green/teal over 1.5s
///    - Elimination: background drains to near-monochrome, progressive panel corruption
///    - Pause: backgroundSpeed decelerates to 0.15x over 0.5s, accelerates on resume
///
///  SCORE & HUD HIERARCHY:
///    - Score: 48px Bold NeonGold, label 11px Dim — 4:1 size contrast
///    - Score flash: brief Color.white 0.08s on each increment
///    - Attempts counter permanent scale-up at last 2 attempts (unrecoverable damage feel)
///    - Lives diamond shatter animation on life loss
///
///  MICRO-ANIMATIONS EVERYWHERE:
///    - Title text breathing: scale 1.0→1.003→1.0 on 4s sine
///    - Numpad dims unused keys on input lock-in
///    - Status bar sweep underline (2px left-to-right, 0.3s, then fade)
///    - Empty input box drip: vertical gradient sweep 8s cycle
///    - Confirm button "lock-in" slide-down-then-spring on arm
///    - Intel log word highlight + dimming as text accumulates
///    - Post-win attempt history replay sequence
///    - Holographic card scan on win verification
///
///  BOOT SCREEN UPGRADES:
///    - Starts on total black, background fades in as systems boot
///    - Per-line state coloring (scanning=amber, loading=white, warning=red)
///    - Terminal cursor blink at typewriter position
///    - Diamond corona glow behind main diamond
///    - Progress bar percentage readout
///
///  INTRO UPGRADES:
///    - Card dealt from off-screen top-right (slide in)
///    - Word-by-word appearance on "YOU HAVE BEEN SELECTED" with dramatic pauses
///    - Mark type text colored in their actual colors during rules reveal
///    - Difficulty options breathe at different individual rates
///
///  INTEL LOG UPGRADE:
///    - History accumulates with older entries dimming to 40% alpha
///    - New entries appear bright then dim as next entry arrives
///    - Scrolls cleanly with visible history of deductive journey
/// </summary>
